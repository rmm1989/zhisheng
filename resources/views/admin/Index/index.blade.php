...
<head>
    ...
    <script src="{{asset('backstage')}}/lib/layui/layui.js" charset="utf-8"></script>
...
<body>
    <!-- 顶部开始 -->
    <div class="container">
        <div class="logo"><a href="./index.html">亿启听web管理端</a></div>
        <div class="left_open">
...
{{--            <a href="javascript:;">{{Auth::guard('admin')->user()->username}}</a>--}}
            <dl class="layui-nav-child"> <!-- 二级菜单 -->
              ...
              <dd><a onclick="x_admin_show('切换帐号','{{url('admin/login')}}')">切换帐号</a></dd>
            ...
            <li>
                ...
                <ul class="sub-menu">
                    @foreach($priv_b as $b)
                        @if($a->id == $b->parent_id)
                            <li>
                                <a _href="{{url($b->address)}}">
                                    <i class="iconfont">&#xe6a7;</i>
                                    <cite>{{$b->priv_name}}</cite>
                                </a>
                            </li>
                        @endif
                    @endforeach
